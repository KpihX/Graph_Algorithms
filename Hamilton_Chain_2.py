...
if not G.isConnected():
    print("\nThe entered graph is not connected and so, can't have an Hamilton chain!")
else:
    Chain = G.HamiltonChain()
    if Chain == ():
        print("\nThe entered graph doesn't have any Hamilton chain!")
    else:
        Chain = "->".join(Chain)
        print(f"\nThere, is an Hamilton chain of the graph entered : {Chain}.")
